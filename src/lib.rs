...
/// * structs may be `Pod` if every member is `Pod` and they have a `repr(C)` or
///   `repr(packed)` attribute to prevent rustc from performing field reordering.
///   The former requires that the supplied data has the correct alignment.
